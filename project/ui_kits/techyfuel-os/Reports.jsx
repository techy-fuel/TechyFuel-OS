...
const REPORTS = [
  ...
  { id: 'content',  name: 'Content engagement report',  desc: 'Reach, engagement and best-performing posts',             icon: 'heart',         tone: ['var(--red-50)',    'var(--red-600)'],    runs: 'Manual'         },
];
...
function fmtAmt(n, currency) {
  const syms = { USD: '$', EUR: '€', GBP: '£', PKR: '₨', AED: 'AED ', SAR: 'SAR ', CAD: 'CA$', AUD: 'A$' };
...
async function runClients(fmt) {
  ...
  const clients = cr.data || [], projects = pr.data || [], invoices = ir.data || [];
  const rows = clients.map(c => {
    const paid = invoices.filter(i => i.client_id === c.id && i.status === 'paid').reduce((s, i) => s + Number(i.amount || 0), 0);
    return { name: c.company || c.name, status: c.status || 'active', proj: projects.filter(p => p.client_id === c.id).length, inv: invoices.filter(i => i.client_id === c.id).length, paid };
  });
  ...
  const sc = { active: 'bs', inactive: 'bn', churned: 'bd' };
  openPDF('Client Performance Report',
    `<div class="grid">
...
async function runRevenue(fmt) {
  ...
  const sc = { paid:'bs', sent:'bn', overdue:'bd', draft:'bn', cancelled:'bn' };
  openPDF('Revenue & Profit Report',
    `<div class="grid">
      <div class="box"><label>Total Invoiced</label><div class="v">$${invoices.reduce((s,i)=>s+Number(i.amount||0),0).toLocaleString()}</div></div>
...
async function runAds(fmt) {
  ...
  const sc = { active:'bs', review:'bw', paused:'bn', ended:'bn', draft:'bn' };
  openPDF('Ads Performance Report',
    `<div class="grid">
...
async function runProjects(fmt) {
  ...
  const projects = pr.data || [], tasks = tr.data || [];
  const rows = projects.map(p => {
    const pt = tasks.filter(t => t.project_id === p.id);
    const done = pt.filter(t => t.status === 'done').length;
    return { name: p.name, client: p.clients?.name||'—', status: p.status, total: pt.length, done, pct: pt.length>0?Math.round((done/pt.length)*100):0 };
  });
  ...
  const sc = { active:'bs', completed:'bs', paused:'bw', cancelled:'bd', draft:'bn' };
  openPDF('Project Status Report',
    `<table><thead><tr><th>Project</th><th>Client</th><th>Status</th><th class="r">Tasks</th><th class="r">Done</th><th class="r">Progress</th></tr></thead><tbody>
...
async function runContent(fmt) {
  ...
  const posts = r.data || [];
  const rows = posts.map(p => ({ title: (p.title||p.caption||'').slice(0,50)||'—', client: p.clients?.name||'—', platform: p.platform||'—', status: p.status||'—', scheduled: p.scheduled_at ? fmtDate(p.scheduled_at) : '—' }));
  ...
  const sc = { published:'bs', scheduled:'bn', draft:'bn', approval:'bw' };
  openPDF('Content Engagement Report',
    `<div class="grid">
...
const RUNNERS = { clients: runClients, team: runTeam, revenue: runRevenue, ads: runAds, projects: runProjects, content: runContent };
...
function ReportCard({ report }) {
  ...
  async function run(fmt) {
    ...
    setLoading(fmt);
    try { await RUNNERS[report.id](fmt); } catch(e) { console.error(e); }
    finally { setLoading(null); }
  }

  const btnBase = { flex: 1, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6, height: 32, background: 'var(--slate-0)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)', fontFamily: 'var(--font-sans)', fontSize: 'var(--text-xs)', fontWeight: 'var(--fw-semibold)', color: 'var(--text-body)', cursor: 'pointer' };

  return (
    <Card padding="md" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <span style={{ width: 42, height: 42, borderRadius: 'var(--radius-lg)', background: report.tone[0], color: report.tone[1], display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
          <Icon name={report.icon} size={21} />
        </span>
        <Badge tone={report.runs === 'Manual' ? 'neutral' : 'success'} size="sm">{report.runs}</Badge>
      </div>
      <div>
        <div style={{ fontSize: 'var(--text-md)', fontWeight: 'var(--fw-bold)', color: 'var(--text-strong)' }}>{report.name}</div>
        <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', marginTop: 4, lineHeight: 1.45 }}>{report.desc}</div>
      </div>
      <div style={{ display: 'flex', gap: 8, paddingTop: 12, borderTop: '1px solid var(--border-subtle)' }}>
        <button onClick={() => run('pdf')} disabled={!!loading} style={{ ...btnBase, opacity: loading && loading !== 'pdf' ? 0.5 : 1, cursor: loading ? 'wait' : 'pointer' }}>
          <Icon name={loading === 'pdf' ? 'loader' : 'file-text'} size={14} style={{ color: 'var(--red-500)' }} />
          {loading === 'pdf' ? 'Generating…' : 'PDF'}
        </button>
        <button onClick={() => run('csv')} disabled={!!loading} style={{ ...btnBase, opacity: loading && loading !== 'csv' ? 0.5 : 1, cursor: loading ? 'wait' : 'pointer' }}>
          <Icon name={loading === 'csv' ? 'loader' : 'sheet'} size={14} style={{ color: 'var(--green-600)' }} />
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1280, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 18, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 'var(--text-3xl)', fontWeight: 'var(--fw-extrabold)', letterSpacing: '-0.02em' }}>Reporting center</h1>
          <p style={{ fontSize: 'var(--text-sm)', color: 'var(--text-muted)', marginTop: 2 }}>6 live reports — export to PDF or Excel</p>
        </div>
      </div>
