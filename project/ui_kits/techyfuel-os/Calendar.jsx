...
function CalendarEvent({ ev }) {
  ...
  return (
    <div title={ev.title} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '2px 5px', borderRadius: 'var(--radius-sm)', background: color + '18', marginBottom: 2, overflow: 'hidden' }}>
      <span style={{ width: 6, height: 6, borderRadius: '50%', background: color, flex: 'none' }} />
      <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-strong)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', lineHeight: 1.3 }}>{ev.title}</span>
    </div>
  );
}

function CalendarCell({ day, year, month, events, isToday, isOtherMonth }) {
...
function Calendar() {
  ...
  const [form, setForm] = React.useState({ title: '', type: 'task', due_date: '', priority: 'medium', status: 'todo', platform: 'instagram', assigned_to: '', client_id: '' });
  ...
  React.useEffect(() => {
    ...
    window.API.getTasks().then(r => {
      ...
      r.data.forEach(t => {
        ...
        addEvent(ds, { id: t.id, title: t.title, type: 'task', priority: t.priority, status: t.status });
        count++;
      });
    ...
    window.API.getContent().then(r => {
      ...
      r.data.forEach(p => {
        ...
        addEvent(ds, { id: p.id, title: p.title, type: 'post', platform: p.platform });
      });
    }).catch(() => {});
  ...
  async function handleAdd() {
    ...
    try {
      if (form.type === 'task') {
        ...
        if (window.API) {
          const { data } = await window.API.createTask(payload);
          if (data && data.due_date) {
            const ds = data.due_date.split('T')[0];
            setEvents(prev => ({ ...prev, [ds]: [...(prev[ds] || []), { id: data.id, title: data.title, type: 'task', priority: data.priority }] }));
          }
        }
      } else {
        const payload = { title: form.title, platform: form.platform, status: 'scheduled' };
        if (form.due_date)  payload.scheduled_at = form.due_date + 'T09:00:00';
        if (form.client_id) payload.client_id    = form.client_id;
        if (window.API) {
          const { data } = await window.API.createPost(payload);
          if (data && data.scheduled_at) {
            const ds = data.scheduled_at.split('T')[0];
            setEvents(prev => ({ ...prev, [ds]: [...(prev[ds] || []), { id: data.id, title: data.title, type: 'post', platform: data.platform }] }));
          }
        }
      }
      setModalOpen(false);
      setForm({ title: '', type: 'task', due_date: '', priority: 'medium', status: 'todo', platform: 'instagram', assigned_to: '', client_id: '' });
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', height: '100%', boxSizing: 'border-box' }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 18, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 'var(--text-3xl)', fontWeight: 'var(--fw-extrabold)', letterSpacing: '-0.02em' }}>Calendar</h1>
          <p style={{ fontSize: 'var(--text-sm)', color: 'var(--text-muted)', marginTop: 2 }}>{monthLabel} · {thisMonthCount} events</p>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 0, height: 36, background: 'var(--slate-0)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)', overflow: 'hidden' }}>
            <button onClick={prevMonth} style={{ width: 36, height: 36, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}><Icon name="chevron-left" size={16} /></button>
            <span style={{ fontSize: 'var(--text-sm)', fontWeight: 'var(--fw-bold)', color: 'var(--text-strong)', padding: '0 10px', borderLeft: '1px solid var(--border-subtle)', borderRight: '1px solid var(--border-subtle)' }}>{monthLabel}</span>
            <button onClick={nextMonth} style={{ width: 36, height: 36, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}><Icon name="chevron-right" size={16} /></button>
          </div>
          <button onClick={() => { setYear(today.getFullYear()); setMonth(today.getMonth()); }} style={{ height: 36, padding: '0 14px', background: 'var(--slate-0)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)', fontFamily: 'var(--font-sans)', fontSize: 'var(--text-sm)', fontWeight: 'var(--fw-semibold)', color: 'var(--text-body)', cursor: 'pointer' }}>Today</button>
      ...
      <Card padding="none" style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', borderBottom: '1px solid var(--border-subtle)' }}>
          {DOW.map(d => (
            <div key={d} style={{ padding: '10px 8px', textAlign: 'center', fontSize: 'var(--text-xs)', fontWeight: 'var(--fw-bold)', letterSpacing: 'var(--tracking-wide)', textTransform: 'uppercase', color: 'var(--text-subtle)', borderRight: '1px solid var(--border-subtle)' }}>{d}</div>
          ))}
        </div>
        <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gridTemplateRows: `repeat(${days.length / 7}, 1fr)`, overflow: 'auto' }}>
          {days.map((day, i) => {
            ...
            return (
              <CalendarCell key={i} day={day} year={year} month={month} events={events} isToday={isToday} isOtherMonth={false} />
            );
          })}
        </div>
      </Card>

      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Add event" onSubmit={handleAdd} loading={saving} submitLabel="Add">
        <FormRow label="Title" required>
          <input style={FF.input} placeholder="Event title…" value={form.title} onChange={e => set('title', e.target.value)} />
        </FormRow>
        <div style={FF.row2}>
          <FormRow label="Type">
            <select style={FF.select} value={form.type} onChange={e => set('type', e.target.value)}>
