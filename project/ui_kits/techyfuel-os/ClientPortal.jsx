...
function mimeIcon(mime) {
  ...
  if (mime.includes('sheet') || mime.includes('excel') || mime.includes('csv')) return { icon: 'table', tone: 'var(--green-600)' };
  if (mime.includes('word') || mime.includes('doc')) return { icon: 'file-text', tone: 'var(--blue-600)' };
...
function getProjectHealth(tasks) {
  if (!tasks || tasks.length === 0) return { label: 'No tasks', color: 'var(--slate-400)' };
  ...
  if (overdue > 0) return { label: 'At risk', color: 'var(--red-500)' };
  if (done / total >= 0.8) return { label: 'On track', color: 'var(--green-600)' };
  return { label: 'In progress', color: 'var(--amber-500)' };
}
...
function ClientPortal() {
  ...
  React.useEffect(() => {
    if (!clientId || !window.API) { setLoading(false); return; }
    setLoading(true);
    ...
    (async () => {
      try {
        ...
        const [pRes, iRes, fRes, cRes, tRes] = await Promise.all([
          ...
          window.API.getFiles({ clientId }).catch(() => ({})),
          window.API.getContent({ clientId }).catch(() => ({})),
        ...
        if (proj) {
          try {
            const { data: taskData } = await window.API.getTasks({ projectId: proj.id });
            setTasks(taskData || []);
          } catch { setTasks([]); }
        } else {
          setTasks([]);
  ...
  async function handleApprove(post) {
    if (!window.API) return;
    try { await window.API.updatePost(post.id, { status: 'published' }); } catch {}
    setApprovals(prev => prev.map(p => p.id === post.id ? { ...p, status: 'published' } : p));
  }

  async function handleReject(post) {
    if (!window.API) return;
    try { await window.API.updatePost(post.id, { status: 'draft' }); } catch {}
  ...
  const milestones   = tasks.slice(0, 6).map(t => ({ label: t.title, state: taskToMilestone(t), due: t.due_date }));
  ...
  if (clients.length === 0) {
    return (
      <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 12, color: 'var(--text-muted)', fontSize: 'var(--text-sm)' }}>
        <Icon name="users" size={36} style={{ opacity: 0.3 }} />
    ...
    <div>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div style={{ position: 'relative' }}>
              <button onClick={() => setClientDrop(d => !d)} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(255,255,255,0.16)', border: '1px solid rgba(255,255,255,0.25)', borderRadius: 'var(--radius-xl)', padding: '8px 14px', cursor: 'pointer', color: '#fff' }}>
                <span style={{ width: 32, height: 32, borderRadius: '50%', background: 'rgba(255,255,255,0.2)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 14 }}>
                  {displayName.charAt(0).toUpperCase()}
                </span>
                <div style={{ textAlign: 'left' }}>
                  <div style={{ fontSize: 'var(--text-2xs)', fontWeight: 'var(--fw-bold)', letterSpacing: 'var(--tracking-caps)', textTransform: 'uppercase', opacity: 0.8 }}>Client portal · preview</div>
                  <div style={{ fontSize: 'var(--text-xl)', fontWeight: 'var(--fw-extrabold)', letterSpacing: '-0.02em' }}>{displayName}</div>
                </div>
                <Icon name="chevrons-up-down" size={16} style={{ opacity: 0.7, marginLeft: 4 }} />
              </button>
              {clientDrop && (
                <div style={{ position: 'absolute', top: 'calc(100% + 8px)', left: 0, zIndex: 200, background: 'var(--slate-0)', border: '1px solid var(--border-subtle)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-xl)', minWidth: 220, overflow: 'hidden' }}
                  onClick={() => setClientDrop(false)}>
                  {clients.map(c => (
                    <div key={c.id} onClick={() => setClientId(c.id)}
                      style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', cursor: 'pointer', background: c.id === clientId ? 'var(--blue-50)' : 'transparent', color: c.id === clientId ? 'var(--blue-700)' : 'var(--text-body)', fontSize: 'var(--text-sm)', fontWeight: c.id === clientId ? 'var(--fw-semibold)' : 'var(--fw-medium)' }}
                      onMouseEnter={e => { if (c.id !== clientId) e.currentTarget.style.background = 'var(--slate-50)'; }}
                      onMouseLeave={e => { if (c.id !== clientId) e.currentTarget.style.background = 'transparent'; }}>
                      <Avatar name={c.company || c.name} size="xs" />
                      <div>
                        <div>{c.company || c.name}</div>
                        {c.email && <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>{c.email}</div>}
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: 24, display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 16 }}>
        ...
        <Card padding="lg" style={{ gridColumn: '1 / -1' }}>
          ...
          {milestones.length === 0 ? (
            <div style={{ padding: '24px 0', textAlign: 'center', color: 'var(--text-muted)', fontSize: 'var(--text-sm)' }}>No tasks assigned to this project yet.</div>
          ) : (
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8 }}>
              {milestones.map((m, i) => (
                <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 9, position: 'relative' }}>
                  ...
                  {m.due && <span style={{ fontSize: 10, color: 'var(--text-subtle)' }}>{fmtDate(m.due)}</span>}
                </div>
              ))}
            </div>
          )}
          ...
          {milestones.length > 0 && (
            <div style={{ marginTop: 20, height: 6, background: 'var(--slate-100)', borderRadius: 99, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: progressPct + '%', background: 'var(--blue-600)', borderRadius: 99, transition: 'width 0.4s ease' }} />
            </div>
          )}
        </Card>
        ...
        <Card padding="lg">
          ...
          {approvals.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {approvals.map((a, i) => {
                const platIcons = { instagram: 'instagram', facebook: 'facebook', linkedin: 'linkedin', youtube: 'youtube', twitter: 'twitter' };
                    ...
                    {isPublished
                      ...
                      : (
                        <div style={{ display: 'flex', gap: 6 }}>
                          ...
                          <button onClick={() => handleReject(a)} title="Send back for revision" style={{ width: 32, height: 32, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: 'var(--slate-0)', color: 'var(--text-muted)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)', cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <Card padding="lg">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              ...
              <span style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>{files.length} file{files.length !== 1 ? 's' : ''}</span>
            </div>
            {files.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {files.slice(0, 5).map((f, i) => {
                  ...
                  return (
                    <div key={f.id || i} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '8px 6px', borderRadius: 'var(--radius-md)' }}
                      ...
                      <button onClick={() => handleFileDownload(f)} title={f.url ? 'Download' : 'No URL available'}
                        style={{ background: 'none', border: 'none', cursor: f.url ? 'pointer' : 'default', padding: 4, color: f.url ? 'var(--blue-600)' : 'var(--text-subtle)', display: 'inline-flex' }}>
                ...
                {files.length > 5 && (
                  <div style={{ textAlign: 'center', paddingTop: 6, fontSize: 'var(--text-xs)', color: 'var(--text-muted)' }}>+{files.length - 5} more files</div>
                )}
              </div>
            )}
          </Card>
          ...
          {invoice && (
            <Card padding="lg" style={{ background: 'var(--slate-900)' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontSize: 'var(--text-2xs)', fontWeight: 'var(--fw-bold)', letterSpacing: 'var(--tracking-caps)', textTransform: 'uppercase', color: 'var(--slate-400)', marginBottom: 4 }}>
                    {invoice.status === 'paid' ? 'Latest invoice (paid)' : 'Invoice due'}
                  </div>
