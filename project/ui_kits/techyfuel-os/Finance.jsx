...
function BrandingModal({ onClose }) {
  ...
  function save() {
    const merged = { ...saved, ...f };
    localStorage.setItem('tf_settings', JSON.stringify(merged));
    try { window.dispatchEvent(new Event('tf-settings-changed')); } catch {}
    onClose();
  }

  const Field = ({ label, k, ph, wide }) => (
    <label style={{ display: 'block', gridColumn: wide ? '1 / -1' : 'auto' }}>
      <span style={{ display: 'block', fontSize: 'var(--text-xs)', fontWeight: 'var(--fw-semibold)', color: 'var(--text-muted)', marginBottom: 4 }}>{label}</span>
      <input style={FF.input} value={f[k] || ''} placeholder={ph} onChange={e => set(k, e.target.value)} />
    </label>
  );
  const sectionTitle = { fontSize: 'var(--text-sm)', fontWeight: 'var(--fw-bold)', color: 'var(--text-strong)', margin: '14px 0 8px' };
  const grid = { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 };

  return (
    <Modal open onClose={onClose} title="Branding & Payment" onSubmit={save} submitLabel="Save changes">
      <p style={{ fontSize: 'var(--text-xs)', color: 'var(--text-muted)', marginTop: -4, marginBottom: 4 }}>
        These details appear on every invoice PDF. Leave a field blank to use the default.
      </p>

      <div style={sectionTitle}>Agency</div>
      <div style={grid}>
        <Field label="Agency name" k="agencyName" ph="TechyFuel" />
        <Field label="Tagline" k="tagline" ph="Fueling Your Brand's Digital Journey" />
        <Field label="Services line" k="servicesLine" ph="Digital Marketing • Web Development • …" wide />
      </div>

      <div style={sectionTitle}>Payment method</div>
      <div style={grid}>
        <Field label="Account #" k="paymentAccount" ph="0123456789012090" />
        <Field label="Swift code" k="paymentSwift" ph="01234560" />
        <Field label="Payoneer" k="paymentPayoneer" ph="payoneer@gmail.com" wide />
      </div>

      <div style={sectionTitle}>Contact</div>
      <div style={grid}>
        <Field label="Phone" k="agencyPhone" ph="+1 (509) 616-0866" />
        <Field label="Web Site" k="agencyWebsite" ph="www.techyfuel.com" />
        <Field label="Email" k="agencyEmail" ph="hello@techyfuel.com" />
        <Field label="Address" k="agencyAddress" ph="America street 20th 201, house no lane 5" />
      </div>

      <div style={sectionTitle}>Signature</div>
      <div style={grid}>
        <Field label="Name" k="signatureName" ph="ZAIN AHMED" />
        <Field label="Title" k="signatureTitle" ph="Founder & CEO" />
      </div>
    </Modal>
  );
}
    ...
    <div style={{ padding: 24, maxWidth: 1280, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 18, flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {activeTab === 'invoices' && (
            <button onClick={() => setBrandingOpen(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, height: 36, padding: '0 14px', background: '#fff', color: 'var(--blue-600)', border: '1px solid var(--border-default)', borderRadius: 'var(--radius-md)', fontFamily: 'var(--font-sans)', fontSize: 'var(--text-sm)', fontWeight: 'var(--fw-semibold)', cursor: 'pointer' }}>
              <Icon name="palette" size={16} /> Branding &amp; Payment
            </button>
          )}
