...
function AreaLine({ data, width = 520, height = 160, color = 'var(--blue-600)', fill = 'rgba(37,99,235,0.10)', id = 'al' }) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} width="100%" preserveAspectRatio="none" style={{ display: 'block', overflow: 'visible' }}>
      <defs><linearGradient id={id} x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stopColor={color} stopOpacity="0.18" /><stop offset="100%" stopColor={color} stopOpacity="0" />
      </linearGradient></defs>
      {[0.25, 0.5, 0.75, 1].map(g => <line key={g} x1="0" x2={width} y1={height * g} y2={height * g} stroke="var(--slate-150)" strokeWidth="1" />)}
      <path d={area} fill={`url(#${id})`} />
      <path d={line} fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
      {pts.map((p, i) => i === pts.length - 1 ? <circle key={i} cx={p[0]} cy={p[1]} r="4" fill={color} stroke="#fff" strokeWidth="2" /> : null)}
    </svg>
  );
}
...
function Bars({ data, labels, width = 520, height = 160, color = 'var(--blue-500)', highlight = 'var(--blue-600)' }) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} width="100%" style={{ display: 'block' }}>
      {[0.33, 0.66, 1].map(g => <line key={g} x1="0" x2={width} y1={height - 20 - (height - 30) * g} y2={height - 20 - (height - 30) * g} stroke="var(--slate-150)" />)}
      {data.map((v, i) => {
        ...
        return (
          <g key={i}>
            <rect x={x} y={height - 20 - h} width={bw} height={h} rx="4" fill={last ? highlight : color} opacity={last ? 1 : 0.5} />
            {labels && <text x={x + bw / 2} y={height - 5} textAnchor="middle" fontSize="10" fill="var(--text-subtle)" fontFamily="var(--font-sans)">{labels[i]}</text>}
          </g>
        );
      })}
    </svg>
...
function Donut({ segments, size = 150, thickness = 22 }) {
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} width={size} height={size}>
      ...
      <text x={size / 2} y={size / 2 - 4} textAnchor="middle" fontSize="26" fontWeight="800" fill="var(--text-strong)" fontFamily="var(--font-display)">{total}</text>
      <text x={size / 2} y={size / 2 + 16} textAnchor="middle" fontSize="11" fill="var(--text-muted)" fontFamily="var(--font-sans)">projects</text>
    </svg>
  );
}
...
function Spark({ data, width = 100, height = 32, color = 'var(--green-500)' }) {
  ...
  return <svg viewBox={`0 0 ${width} ${height}`} width="100%" height={height} preserveAspectRatio="none"><path d={line} fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>;
}
