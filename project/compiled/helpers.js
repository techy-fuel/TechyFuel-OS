...
function readIntegrationsCfg() {
  try {
    return JSON.parse(localStorage.getItem('tf_integrations') || '{}');
  } catch {
    return {};
  }
}
...
async function pickFilesFromGoogleDrive({
  ...
  await Promise.all([loadScriptOnce('https://accounts.google.com/gsi/client'), loadScriptOnce('https://apis.google.com/js/api.js')]);
