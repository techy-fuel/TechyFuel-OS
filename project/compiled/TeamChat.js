  ...
  function CallModal({
    ...
    React.useEffect(() => {
      ...
      function join(url) {
        ...
        frame.on('participant-joined', () => setParticipants(p => {
          participantsRef.current = p + 1;
          return p + 1;
        }));
        frame.on('participant-left', () => setParticipants(p => {
      ...
      function requestRoom() {
        fetch('/api/daily-room', {
          ...
          if (cancelled) return;
          if (!url) {
            setError(err || 'Could not start call');
            return;
          }
          join(url);
        }).catch(() => {
          if (!cancelled) setError('Could not reach call server');
        });
      }
    ...
    }, "Connecting to call…")), error && /*#__PURE__*/React.createElement("div", {
      style: {
        position: 'absolute',
        inset: 0,
        display: 'flex',
    ...
    }, "Could not start call"), /*#__PURE__*/React.createElement("p", {
      style: {
        fontSize: 13,
        maxWidth: 360
      }
    }, error))), /*#__PURE__*/React.createElement("style", null, `@keyframes spin { to { transform: rotate(360deg); } }`));
  }
